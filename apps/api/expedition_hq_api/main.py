...
def agents() -> list[dict[str, Any]]:
    ...
    audited_events = audit_event_claims(rows("events"), known_agent_ids=known_agent_ids)
    xp_statuses = aggregate_agent_xp_status(audited_events, known_agent_ids)
    return [
        {**agent, "xp_status": xp_statuses.get(agent["id"])}
...
def events() -> list[dict[str, Any]]:
    ...
    audited_events = audit_event_claims(rows("events"), known_agent_ids=known_agent_ids)
    return sorted(audited_events, key=lambda e: e.get("timestamp", ""), reverse=True)
